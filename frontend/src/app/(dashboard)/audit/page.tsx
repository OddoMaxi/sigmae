...
export default function AuditPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[22px] font-bold text-[color:var(--color-navy-900)] tracking-tight">Journal d'audit</h1>
        <p className="text-sm text-slate-500">Historique complet de toutes les actions du système</p>
      </div>
      ...
      <div className="bg-white rounded-[var(--radius-card)] border border-slate-200/70 overflow-hidden">
              ...
              {data?.data?.map((log: any) => (
                <tr key={log.id} className="border-b hover:bg-slate-50 transition">
                  ...
                  <td className="px-4 py-3">
                    <p className="font-medium text-slate-700">{log.user?.name || 'Système'}</p>
        ...
        {data?.last_page > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t">
            <p className="text-xs text-slate-500">{data.from}–{data.to} sur {data.total}</p>
