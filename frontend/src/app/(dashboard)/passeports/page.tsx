  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[22px] font-bold text-[color:var(--color-navy-900)] tracking-tight">Passeports</h1>
          <p className="text-sm text-slate-500">Gestion du stock et suivi des passeports</p>
        </div>
      ...
      {/* Filtres */}
      <div className="bg-white rounded-[var(--radius-card)] border border-slate-200/70 p-4 flex gap-4">
        ...
        <select
          ...
          className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a5276]"
        >
          {STATUTS.map((s) => (
                ...
                {(data?.data ?? []).map((p: any) => (
                  <tr key={p.id} className="border-b hover:bg-slate-50 transition">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-slate-400">{p.telephone || p.email_citoyen || ''}</p>
                    </td>
                    <td className="px-4 py-3 text-slate-500 text-sm">{p.email_citoyen || '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${statutStyle[p.statut] ?? 'bg-slate-100 text-slate-600'}`}>
                    ...
                    <td className="px-4 py-3">
                      {p.ambassade_destination ? (
                        <div>
                          ...
                          <p className="text-xs text-slate-400">{p.ambassade_destination.code}</p>
                        </div>
                      ) : p.lot ? (
                        <div>
